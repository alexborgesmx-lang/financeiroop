...
function parseDate(v){
  ...
  if(typeof v === "string"){
    ...
    if(s.includes("/")){
      ...
      let ano = parseInt(partes[0]), mes = parseInt(partes[1]), dia = parseInt(partes[2]);
      if(ano < 100) ano += 2000;
      d = new Date(ano, mes - 1, dia, 12, 0, 0);
    } else { d = new Date(v); }
  } else { d = new Date(v); }
...
function BaixaModal({contrato, parcelas, onConfirmar, onFechar}){
  ...
  const jurosJaPagos=ps.filter(p=>p.STATUS==="pago").reduce((s,p)=>{const pago=parseFloat(p.VALOR_PAGO)||0;const princ=parseFloat(p.VALOR_PRINCIPAL)||0;return s+Math.max(0,pago-princ);},0);
  ...
  const diasAtraso=ps.filter(p=>p.STATUS==="atrasado").length>0?Math.max(...ps.filter(p=>p.STATUS==="atrasado").map(p=>{const dv=parseDate(p.DATA_VENCIMENTO);if(!dv)return 0;const d=Math.round((new Date()-dv)/86400000);return d>0?d:0;})):0;
  const confirmar=async()=>{if(!dados.motivo){setMsg({ok:false,texto:"Informe o motivo da baixa."});return;}setLoading(true);setMsg(null);const res=await postAction({action:"baixarContrato",idContrato:contrato.ID_CONTRATO,dados:{...dados,diasAtraso,valorRecuperadoAntesBaixa:capitalRecuperado,jurosJaRecebidos:jurosJaPagos,data:hojeStr()}});if(res.ok){onConfirmar();}else setMsg({ok:false,texto:res.erro||"Erro."});setLoading(false);};
  const campo=(label,field,options)=>(<div><span style={LS}>{label}</span>{options?<select value={dados[field]} onChange={e=>setDados(p=>({...p,[field]:e.target.value}))} style={IS}>{options.map(o=><option key={o.v} value={o.v}>{o.l}</option>)}</select>:<input value={dados[field]} onChange={e=>setDados(p=>({...p,[field]:e.target.value}))} style={IS}/>}</div>);
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><div style={{background:CARD,borderRadius:12,width:"100%",maxWidth:600,maxHeight:"90vh",display:"flex",flexDirection:"column",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}><div style={{padding:"20px 24px 14px",borderBottom:`1px solid ${BD}`,background:"#FEF2F2",borderRadius:"12px 12px 0 0"}}><h2 style={{color:RED,fontSize:17,fontWeight:700,margin:0}}>⚠️ Baixar Contrato como Prejuízo</h2><p style={{fontSize:13,color:MUTED,margin:"4px 0 0"}}>Contrato {contrato.ID_CONTRATO} • {contrato.NOME_CLIENTE}</p></div><div style={{padding:24,overflowY:"auto",flex:1}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:24,padding:16,background:BG,borderRadius:10}}><div><span style={LS}>Capital Emprestado</span><div style={{fontSize:15,fontWeight:700}}>{fmtR(valPrincipal)}</div></div><div><span style={LS}>Capital Recuperado</span><div style={{fontSize:15,fontWeight:700,color:GRN}}>{fmtR(capitalRecuperado)}</div></div><div style={{borderTop:`1px solid ${BD}`,paddingTop:8}}><span style={LS}>Prejuízo de Capital</span><div style={{fontSize:15,fontWeight:700,color:RED}}>{fmtR(prejuizoCapital)}</div></div><div style={{borderTop:`1px solid ${BD}`,paddingTop:8}}><span style={LS}>Juros Não Realizados</span><div style={{fontSize:15,fontWeight:700,color:ORG}}>{fmtR(jurosNaoReal)}</div></div><div style={{borderTop:`1px solid ${BD}`,paddingTop:8}}><span style={LS}>Total Pago</span><div style={{fontSize:15,fontWeight:700,color:BLU}}>{fmtR(totalPago)}</div></div><div style={{borderTop:`1px solid ${BD}`,paddingTop:8}}><span style={LS}>% Recuperado</span><div style={{fontSize:15,fontWeight:700,color:RED}}>{fmtP(pctRecuperado)}</div></div><div style={{gridColumn:"1/-1",borderTop:`1px solid ${BD}`,paddingTop:8}}><span style={LS}>Dias de Atraso (Máx)</span><div style={{fontSize:15,fontWeight:700,color:RED}}>{diasAtraso} dias</div></div></div><div style={{display:"flex",flexDirection:"column",gap:16}}>{campo("Substatus / Motivo Operacional","substatus",[{v:"CLIENTE_DESAPARECIDO",l:"Cliente Desaparecido / Sem Contato"},{v:"SEM_BENS_PENHORAVEIS",l:"Sem Bens ou Renda para Cobrança"},{v:"FALECIMENTO",l:"Falecimento do Titular"},{v:"FRAUDE_IDENTIFICADA",l:"Má-fé aparente"},{v:"ACORDO_VALOR_IRRISORIO",l:"Acordo (Valor Irrisório para Prosseguir)"}])}{campo("Motivo Detalhado (Obrigatório)","motivo")}{campo("Possibilidade de Recuperação Futura","possibilidadeRecuperacao",[{v:"BAIXA",l:"Baixa — cliente pouco responsivo"},{v:"RECURSOS_FUTUROS",l:"Remota (Monitorar Renda Futura)"},{v:"JUDICIAL",l:"Judicial (Enviar para Advogado)"}])}{campo("Status Jurídico","statusJuridico",[{v:"NAO_ANALISADO",l:"Não analisado"},{v:"ANALISE_INTERNA",l:"Análise Interna"},{v:"PROCESSO_AJUIZADO",l:"Processo Ajuizado"}])}<div><span style={LS}>Próxima Providência</span><input value={dados.proximaProvidencia} onChange={e=>setDados(p=>({...p,proximaProvidencia:e.target.value}))} style={IS}/></div><div><span style={LS}>Observação Adicional</span><textarea value={dados.observacao} onChange={e=>setDados(p=>({...p,observacao:e.target.value}))} style={{...IS,height:80,resize:"none"}}/></div></div>{msg&&<div style={{marginTop:16,padding:12,borderRadius:8,background:RED+"10",color:RED,fontSize:13,textAlign:"center",fontWeight:600}}>{msg.texto}</div>}</div><div style={{padding:20,borderTop:`1px solid ${BD}`,display:"flex",gap:12}}><button onClick={onFechar} style={{flex:1,padding:12,borderRadius:8,border:`1px solid ${BD}`,background:CARD,cursor:"pointer",fontWeight:600}}>Cancelar</button><button onClick={confirmar} disabled={loading} style={{flex:2,padding:12,borderRadius:8,border:"none",background:RED,color:"#FFF",cursor:"pointer",fontWeight:700,opacity:loading?0.7:1}}>{loading?"Processando...":"Confirmar Baixa como Prejuízo"}</button></div></div></div>);
}
...
function ModalAcordoPerda({contrato,parcelas,onConfirmar,onFechar}){
  ...
  const confirmar=async()=>{if(!valorAcordo||vAcordo<=0){setMsg("Informe o valor acordado.");return;}setLoading(true);const res=await postAction({action:"baixarContrato",idContrato:contrato.ID_CONTRATO,dados:{tipo:"acordo_com_perda",valorRecebido:vAcordo,principalPerdido,jurosCancelados,motivo:"Liquidação com desconto/acordo",substatus:"ACORDO_LIQUIDACAO",data:hojeStr()}});if(res.ok)onConfirmar();else setMsg(res.erro||"Erro ao registrar acordo.");setLoading(false);};
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><div style={{background:"#FFF",borderRadius:16,width:"100%",maxWidth:500,padding:24,boxShadow:"0 20px 50px rgba(0,0,0,0.3)"}}><h2 style={{margin:"0 0 10px",color:RED,fontSize:20}}>🤝 Acordo de Encerramento</h2><p style={{fontSize:14,color:MUTED,marginBottom:20}}>Contrato: <strong>{contrato.ID_CONTRATO}</strong> • {contrato.NOME_CLIENTE}</p><div style={{background:BG,padding:15,borderRadius:10,marginBottom:20}}><div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:5}}><span>Principal em Aberto:</span><strong>{fmtR(principalAberto)}</strong></div><div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:5}}><span>Juros Futuros:</span><strong>{fmtR(jurosAberto)}</strong></div><div style={{display:"flex",justifyContent:"space-between",fontSize:14,fontWeight:700,borderTop:`1px solid ${BD}`,paddingTop:5}}><span>Total Contratual:</span><span>{fmtR(totalAberto)}</span></div></div><span style={LS}>Valor Recebido no Acordo</span><input type="number" value={valorAcordo} onChange={e=>setValorAcordo(e.target.value)} placeholder="0.00" style={{...IS,fontSize:18,fontWeight:700,height:50,textAlign:"center",borderColor:RED}}/><div style={{marginTop:20,padding:15,background:RED+"08",borderRadius:10,border:`1px dashed ${RED}30`}}><div style={{display:"flex",justifyContent:"space-between",fontSize:13,color:RED}}><span>Prejuízo Real (Principal):</span><strong>{fmtR(principalPerdido)}</strong></div><div style={{display:"flex",justifyContent:"space-between",fontSize:13,color:MUTED,marginTop:5}}><span>Juros Cancelados:</span><strong>{fmtR(jurosCancelados)}</strong></div></div>{msg&&<div style={{marginTop:15,color:RED,fontSize:13,textAlign:"center"}}>{msg}</div>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:24}}><button onClick={onFechar} style={{padding:12,borderRadius:8,border:`1px solid ${BD}`,background:"none",cursor:"pointer"}}>Cancelar</button><button onClick={confirmar} disabled={loading} style={{padding:12,borderRadius:8,border:"none",background:RED,color:"#FFF",fontWeight:700,cursor:"pointer",opacity:loading?0.7:1}}>{loading?"Processando...":"Confirmar Acordo"}</button></div></div></div>);
}
...
function RecuperacaoModal({contrato,onConfirmar,onFechar}){
  ...
  const confirmar=async()=>{if(!valor)return;setLoading(true);setMsg(null);const res=await postAction({action:"recuperacaoAposBaixa",idContrato:contrato.ID_CONTRATO,dados:{valor:parseFloat(valor),data:hojeStr()}});if(res.ok)onConfirmar();else setMsg(res.erro||"Erro.");setLoading(false);};
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><div style={{background:CARD,borderRadius:12,width:"100%",maxWidth:400,padding:24,boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}><h2 style={{fontSize:18,fontWeight:700,margin:"0 0 8px",color:PUR}}>💸 Recuperação de Crédito</h2><p style={{fontSize:13,color:MUTED,margin:"0 0 20px"}}>Registrar entrada para contrato já baixado: {contrato.ID_CONTRATO}</p><div style={{marginBottom:20}}><span style={LS}>Valor Recebido</span><input type="number" value={valor} onChange={e=>setValor(e.target.value)} placeholder="0.00" style={{...IS,fontSize:16,fontWeight:700}}/></div>{msg&&<div style={{marginBottom:16,color:RED,fontSize:13}}>{msg}</div>}<div style={{display:"flex",gap:12}}><button onClick={onFechar} style={{flex:1,padding:10,borderRadius:8,border:`1px solid ${BD}`,background:CARD,cursor:"pointer"}}>Sair</button><button onClick={confirmar} disabled={loading} style={{flex:2,padding:10,borderRadius:8,border:"none",background:PUR,color:"#FFF",cursor:"pointer",fontWeight:700,opacity:loading?0.7:1}}>{loading?"Gravando...":"Confirmar Recebimento"}</button></div></div></div>);
}

function ClienteModal({cliente,onFechar,onSucesso}){
  const [tab,setTab]=useState("perfil");
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><div style={{background:BG,borderRadius:16,width:"100%",maxWidth:1000,height:"90vh",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 30px 90px rgba(0,0,0,0.3)"}}><div style={{background:CARD,padding:24,borderBottom:`1px solid ${BD}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:16}}><div style={{width:48,height:48,background:BLU,borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",color:"#FFF",fontSize:20,fontWeight:800}}>{cliente.NOME_CLIENTE?.charAt(0)||"?"}</div><div><h2 style={{margin:0,fontSize:20,fontWeight:800}}>{cliente.NOME_CLIENTE}</h2><div style={{fontSize:13,color:MUTED}}>{cliente.ID_CLIENTE} • {cliente.CPF||cliente.CPF_CNPJ}</div></div></div><button onClick={onFechar} style={{background:BG,border:"none",width:32,height:32,borderRadius:8,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{Ico.arr}</button></div><div style={{display:"flex",background:CARD,padding:"0 24px",borderBottom:`1px solid ${BD}`,gap:24}}>{["perfil","contratos","historico"].map(t=><button key={t} onClick={()=>setTab(t)} style={{padding:"15px 5px",background:"none",border:"none",borderBottom:tab===t?`2px solid ${BLU}`:"2px solid transparent",color:tab===t?BLU:MUTED,fontWeight:600,cursor:"pointer",fontSize:13,textTransform:"capitalize"}}>{t}</button>)}</div><div style={{flex:1,overflowY:"auto",padding:24}}>{tab==="perfil"&&(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24}}><div style={{background:CARD,padding:20,borderRadius:12,border:`1px solid ${BD}`}}><h3 style={{margin:"0 0 15px",fontSize:15}}>Dados Pessoais</h3><div style={{display:"grid",gap:12}}>{[{l:"Celular",v:cliente.TELEFONE||cliente.TELEFONE_WPP},{l:"Cidade",v:cliente.CIDADE_ESTADO||((cliente.CIDADE||"")+" - "+(cliente.UF||""))},{l:"Endereço",v:cliente.ENDERECO||cliente.RUA}].map(i=><div key={i.l}><span style={LS}>{i.l}</span><div style={{fontSize:14}}>{i.v||"—"}</div></div>)}</div></div><div style={{background:CARD,padding:20,borderRadius:12,border:`1px solid ${BD}`}}><h3 style={{margin:"0 0 15px",fontSize:15}}>Informações de Crédito</h3><div style={{display:"grid",gap:12}}>{[{l:"Status",v:<Badge c={cliente.STATUS_CLIENTE==="ativo"?GRN:YEL}>{cliente.STATUS_CLIENTE?.toUpperCase()}</Badge>},{l:"Score Interno",v:cliente.SCORE||"N/A"}].map(i=><div key={i.l}><span style={LS}>{i.l}</span><div style={{fontSize:14}}>{i.v}</div></div>)}</div></div></div>)}</div></div></div>);
}

function PagamentoDrop({contratos,parcelas,onSucesso}){
  const [busca,setBusca]=useState("");const [showDrop,setShowDrop]=useState(false);const [cliente,setCliente]=useState(null);const [parcela,setParcela]=useState(null);const [tipo,setTipo]=useState("total");const [data,setData]=useState(hojeStr());const [valor,setValor]=useState("");const [loading,setLoading]=useState(false);const [msg,setMsg]=useState(null);const ref=useRef();
  ...
  return(<div style={{background:CARD,borderRadius:12,padding:24,border:`1px solid ${BD}`,boxShadow:"0 4px 20px rgba(0,0,0,0.05)"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}><div style={{background:GRN+"15",color:GRN,padding:8,borderRadius:8}}>{Ico.pag}</div><h3 style={{margin:0,fontSize:16,fontWeight:700}}>Registrar Pagamento</h3></div><div style={{display:"flex",flexDirection:"column",gap:16}}><div style={{position:"relative"}} ref={ref}><span style={LS}>Buscar Cliente</span><div style={{position:"relative"}}><div style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)"}}>{Ico.srch}</div><input value={cliente?cliente.NOME_CLIENTE:busca} onChange={e=>{setBusca(e.target.value);setCliente(null);setParcela(null);setShowDrop(true);}} onFocus={()=>setShowDrop(true)} placeholder="Nome ou ID do cliente..." style={{...IS,paddingLeft:35}}/>{cliente&&<button onClick={()=>{setCliente(null);setBusca("");}} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",border:"none",background:"none",cursor:"pointer",color:MUTED}}>x</button>}</div>{showDrop&&clis.length>0&&(<div style={{position:"absolute",top:"100%",left:0,right:0,background:CARD,border:`1px solid ${BD}`,borderRadius:8,marginTop:4,zIndex:100,boxShadow:"0 10px 30px rgba(0,0,0,0.1)",overflow:"hidden"}}>{clis.map(c=><div key={c.ID_CLIENTE} onClick={()=>{setCliente(c);setShowDrop(false);}} style={{padding:"10px 15px",cursor:"pointer",borderBottom:`1px solid ${BG}`,fontSize:13}} onMouseEnter={e=>e.target.style.background=BG} onMouseLeave={e=>e.target.style.background=CARD}><strong>{c.ID_CLIENTE}</strong> - {c.NOME_CLIENTE}</div>)}</div>)}</div>{cliente&&(<div><span style={LS}>Parcela Pendente</span>{pars.length>0?<select value={parcela?.ID_PARCELA||""} onChange={e=>{const p=pars.find(x=>x.ID_PARCELA===e.target.value);setParcela(p);setValor(p.VALOR_PARCELA);}} style={IS}><option value="">Selecione...</option>{pars.map(p=><option key={p.ID_PARCELA} value={p.ID_PARCELA}>Parc {p.NUM_PARCELA} ({fmtDt(p.DATA_VENCIMENTO)}) - {fmtR(p.VALOR_PARCELA)}</option>)}</select>:<div style={{padding:10,background:RED+"08",color:RED,fontSize:12,borderRadius:6,fontWeight:600}}>Nenhuma parcela pendente.</div>}</div>)}{parcela&&(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><div><span style={LS}>Tipo</span><select value={tipo} onChange={e=>setTipo(e.target.value)} style={IS}><option value="total">Total</option><option value="parcial">Somente Juros (Rolar Principal)</option></select></div><div><span style={LS}>Valor</span><input type="number" value={valor} onChange={e=>setValor(e.target.value)} style={IS}/></div><div style={{gridColumn:"1/-1"}}><span style={LS}>Data Pagamento</span><input type="date" value={data} onChange={e=>setData(e.target.value)} style={IS}/></div><button onClick={registrar} disabled={loading} style={{gridColumn:"1/-1",marginTop:8,padding:"12px",borderRadius:8,border:"none",background:GRN,color:"#FFF",fontWeight:700,cursor:"pointer",opacity:loading?0.7:1}}>{loading?"Processando...":"Confirmar Pagamento"}</button></div>)}{msg&&<div style={{padding:12,borderRadius:8,background:msg.ok?GRN+"10":RED+"10",color:msg.ok?GRN:RED,fontSize:13,textAlign:"center",fontWeight:600}}>{msg.t}</div>}</div></div>);
}
...
function NovoContrato({contratos,onSucesso}){
  ...
  useEffect(()=>{const h=e=>{if(ref.current&&!ref.current.contains(e.target))setShowDrop(false);};document.addEventListener("mousedown",h);return()=>document.removeEventListener("mousedown",h);},[]);
  const clis=useMemo(()=>{if(busca.length<2)return[];const ids=new Set();return contratos.filter(c=>{const m=c.NOME_CLIENTE.toLowerCase().includes(busca.toLowerCase())||c.ID_CLIENTE.toLowerCase().includes(busca.toLowerCase());if(m&&!ids.has(c.ID_CLIENTE)){ids.add(c.ID_CLIENTE);return true;}return false;}).slice(0,6);},[busca,contratos]);
  const criar=async()=>{if(!cliente||!principal||!parcelas||!taxa||!dtEmp||!dtVenc)return;setLoading(true);setMsg(null);const res=await postAction({action:"novoContrato",dados:{idCliente:cliente.ID_CLIENTE,nomeCliente:cliente.NOME_CLIENTE,principal,parcelas,taxa,dataEmprestimo:dtEmp,dataVencimento:dtVenc}});if(res.ok){setMsg({ok:true,t:"Contrato criado!"});setTimeout(onSucesso,1500);}else setMsg({ok:false,t:res.erro||"Erro"});setLoading(false);};
  return(<div style={{background:CARD,borderRadius:12,padding:24,border:`1px solid ${BD}`,boxShadow:"0 4px 20px rgba(0,0,0,0.05)"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}><div style={{background:BLU+"15",color:BLU,padding:8,borderRadius:8}}>{Ico.ctr}</div><h3 style={{margin:0,fontSize:16,fontWeight:700}}>Novo Contrato</h3></div><div style={{display:"flex",flexDirection:"column",gap:16}}><div style={{position:"relative"}} ref={ref}><span style={LS}>Buscar Cliente</span><div style={{position:"relative"}}><div style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)"}}>{Ico.srch}</div><input value={cliente?cliente.NOME_CLIENTE:busca} onChange={e=>{setBusca(e.target.value);setCliente(null);setShowDrop(true);}} onFocus={()=>setShowDrop(true)} placeholder="Nome ou ID..." style={{...IS,paddingLeft:35}}/>{cliente&&<button onClick={()=>{setCliente(null);setBusca("");}} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",border:"none",background:"none",cursor:"pointer",color:MUTED}}>x</button>}</div>{showDrop&&clis.length>0&&(<div style={{position:"absolute",top:"100%",left:0,right:0,background:CARD,border:`1px solid ${BD}`,borderRadius:8,marginTop:4,zIndex:100,boxShadow:"0 10px 30px rgba(0,0,0,0.1)",overflow:"hidden"}}>{clis.map(c=><div key={c.ID_CLIENTE} onClick={()=>{setCliente(c);setShowDrop(false);}} style={{padding:"10px 15px",cursor:"pointer",borderBottom:`1px solid ${BG}`,fontSize:13}} onMouseEnter={e=>e.target.style.background=BG} onMouseLeave={e=>e.target.style.background=CARD}><strong>{c.ID_CLIENTE}</strong> - {c.NOME_CLIENTE}</div>)}</div>)}</div>{cliente&&(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><div><span style={LS}>Valor Principal</span><input type="number" value={principal} onChange={e=>setPrincipal(e.target.value)} placeholder="0.00" style={IS}/></div><div><span style={LS}>Nº Parcelas</span><input type="number" value={parcelas} onChange={e=>setParcelas(e.target.value)} placeholder="1" style={IS}/></div><div><span style={LS}>Taxa Mensal (%)</span><input type="number" value={taxa} onChange={e=>setTaxa(e.target.value)} placeholder="0.00" style={IS}/></div><div><span style={LS}>1º Vencimento</span><input type="date" value={dtVenc} onChange={e=>setDtVenc(e.target.value)} style={IS}/></div><div style={{gridColumn:"1/-1"}}><span style={LS}>Data Empréstimo</span><input type="date" value={dtEmp} onChange={e=>setDtEmp(e.target.value)} style={IS}/></div><button onClick={criar} disabled={loading} style={{gridColumn:"1/-1",marginTop:8,padding:"12px",borderRadius:8,border:"none",background:BLU,color:"#FFF",fontWeight:700,cursor:"pointer",opacity:loading?0.7:1}}>{loading?"Criando...":"Gerar Contrato"}</button></div>)}{msg&&<div style={{padding:12,borderRadius:8,background:msg.ok?GRN+"10":RED+"10",color:msg.ok?GRN:RED,fontSize:13,textAlign:"center",fontWeight:600}}>{msg.t}</div>}</div></div>);
}

// ── CALENDÁRIO DE RANGE REUTILIZÁVEL ──────────────────────────────
function CalendarioRange({de, ate, onSelecionar, onLimpar}){
  const [mes, setMes] = useState(()=>{ const d=new Date(); d.setDate(1); return d; });
  const [step, setStep] = useState(0); // 0=início, 1=fim
  ...
  function clicar(d){
    if(step===0){ onSelecionar(d, null); setStep(1); }
      ...
      if(d < de){ onSelecionar(d, de); } else { onSelecionar(de, d); }
      setStep(0);
    }
  }
  ...
  return(
    <div style={{background:CARD,border:`1px solid ${BD}`,borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,0.14)",padding:16,minWidth:300}}>
      ...
      {(de||ate)&&<div style={{marginTop:12,paddingTop:10,borderTop:`1px solid ${BD}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
...
function App() {
  ...
  const pFiltradas=useMemo(()=>contratos.filter(c=>{const p=STATUS_PERDA.includes(c.STATUS_CONTRATO);const s=filtroPerdas==="todos"||c.STATUS_CONTRATO===filtroPerdas;return p&&s;}),[contratos,filtroPerdas]);
  ...
  // Pagamentos filtrados por período (aba Financeiro)
  const pagsFiltrados = useMemo(()=>{
    ...
    return sorted.filter(p=>{
      ...
      if(!d) return false;
      d.setHours(0,0,0,0);
      const fim = finAte ? new Date(finAte) : new Date(finDe);
  ...
  const labelPeriodo = finDe && finAte ? `${dStrFin(finDe)} → ${dStrFin(finAte)}` : finDe ? `A partir de ${dStrFin(finDe)}` : "Filtrar por período";
    ...
    <div style={{display:"flex",height:"100vh",background:BG,color:TEXT,fontFamily:"'Inter', sans-serif"}}>
      ...
      <div style={{width:sidebarOpen?SW:0,background:CARD,borderRight:`1px solid ${BD}`,display:"flex",flexDirection:"column",overflow:"hidden",transition:"0.3s",flexShrink:0}}>
        ...
        <div style={{padding:16,flex:1}}>
          ...
          <NavItem id="financeiro" label="Financeiro"        ico={Ico.fin}/>
          <NavItem id="perdas"     label="Perdas & Recup."   ico={Ico.loss}/>
          ...
          {tab==="dashboard"&&(
            <div style={{display:"flex",flexDirection:"column",gap:24}}>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(240px, 1fr))",gap:20}}>
                {[{l:"Total em Aberto",v:fmtR(M.vAtivos),c:BLU,i:Ico.fin},{l:"Total em Atraso",v:fmtR(M.vAtrasoTotal),c:YEL,i:Ico.cob},{l:"Inadimplência",v:fmtP(M.taxaInad),c:M.taxaInad>15?RED:M.taxaInad>8?YEL:GRN,i:Ico.kpi},{l:"Lucro Projetado",v:fmtR(M.lucroTotal),c:GRN,i:Ico.novo}].map(k=>(
                  <div key={k.l} style={{background:CARD,padding:20,borderRadius:12,border:`1px solid ${BD}`,display:"flex",alignItems:"center",gap:16}}><div style={{background:k.c+"15",color:k.c,padding:12,borderRadius:12}}>{k.i}</div><div><div style={{fontSize:12,color:MUTED,fontWeight:600,marginBottom:4}}>{k.l}</div><div style={{fontSize:20,fontWeight:800}}>{k.v}</div></div></div>
                ))}
              </div>
          ...
          {tab==="clientes"&&(
            <div style={{background:CARD,borderRadius:12,border:`1px solid ${BD}`,overflow:"hidden"}}>
              ...
              <table style={{width:"100%",borderCollapse:"collapse",textAlign:"left"}}>
                <thead><tr style={{background:BG,fontSize:11,color:MUTED,textTransform:"uppercase",letterSpacing:"0.05em"}}><th style={{padding:"12px 20px"}}>Cliente</th><th>Status</th><th>Telefone</th><th style={{padding:"12px 20px",textAlign:"right"}}>Ações</th></tr></thead>
                <tbody>{filtrados.map(c=>(
                  <tr key={c.ID_CLIENTE} style={{borderBottom:`1px solid ${BD}`,fontSize:13}} onMouseEnter={e=>e.currentTarget.style.background=BG+"30"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                    <td style={{padding:"15px 20px"}}><div style={{fontWeight:700}}>{c.NOME_CLIENTE||c.NOME}</div><div style={{fontSize:11,color:MUTED}}>{c.ID_CLIENTE}</div></td>
                    <td><Badge c={c.STATUS_CLIENTE==="ativo"?GRN:YEL}>{c.STATUS_CLIENTE?.toUpperCase()}</Badge></td>
                    <td style={{color:MUTED,fontSize:13}}>{c.TELEFONE||c.TELEFONE_WPP||"—"}</td>
              ...
              <table style={{width:"100%",borderCollapse:"collapse",textAlign:"left"}}>
                ...
                <tbody>{cobItems.map(c=>(
                  <tr key={c.ID_CLIENTE} style={{borderBottom:`1px solid ${BD}`,fontSize:13}}>
                    <td style={{padding:"15px 20px"}}><div style={{fontWeight:700}}>{c.NOME_CLIENTE||c.NOME}</div><div style={{fontSize:11,color:MUTED}}>{c.TELEFONE||c.TELEFONE_WPP}</div></td>
          ...
          {tab==="financeiro"&&(
            <div style={{display:"flex",flexDirection:"column",gap:24}}>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
                  <div>
                    ...
                    <p style={{margin:"4px 0 0",fontSize:12,color:MUTED}}>
                      {finDe ? `Exibindo ${totaisFin.count} pagamento(s) — ${labelPeriodo}` : `Todos os ${pagamentos.length} pagamentos registrados`}
                    </p>
                  </div>
                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                    {finDe&&<button onClick={()=>{setFinDe(null);setFinAte(null);}} style={{padding:"7px 14px",borderRadius:8,border:`1px solid ${BD}`,background:CARD,color:MUTED,fontSize:12,cursor:"pointer",fontWeight:600}}>✕ Limpar filtro</button>}
                    <div style={{position:"relative"}}>
                      <button onClick={()=>setFinCalOpen(o=>!o)} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,border:`1.5px solid ${finDe?BLU:BD}`,background:finDe?"#EFF6FF":CARD,color:finDe?BLU:MUTED,fontSize:13,fontWeight:finDe?700:400,cursor:"pointer"}}>
                        {Ico.cal} {finDe ? labelPeriodo : "Selecionar período"}
                      </button>
                      {finCalOpen&&(
              ...
              <div style={{background:CARD,borderRadius:12,border:`1px solid ${BD}`,overflow:"hidden"}}>
                <table style={{width:"100%",borderCollapse:"collapse",textAlign:"left"}}>
                  ...
                  <tbody>
                    {pagsFiltrados.length===0
                      ? <tr><td colSpan={7} style={{padding:"32px 20px",textAlign:"center",color:MUTED,fontSize:13}}>Nenhum pagamento encontrado neste período.</td></tr>
                      : pagsFiltrados.map((p,i)=>{
                          ...
                          const tLabel={pagamento_normal:"Normal",pagamento_com_atraso:"Com Atraso",somente_juros:"Só Juros",recuperacao_apos_baixa:"Recuperação"}[p.TIPO_PAGAMENTO]||p.TIPO_PAGAMENTO||"—";
                          ...
                          return(
                            <tr key={i} style={{borderBottom:`1px solid ${BD}`,fontSize:13,background:i%2===0?CARD:"#FAFAFA"}}>
                              <td style={{padding:"12px 20px",color:MUTED,whiteSpace:"nowrap"}}>{fmtDt(parseDate(p.DATA_PAGAMENTO))}</td>
                ...
                {!finDe&&<div style={{padding:"10px 20px",borderTop:`1px solid ${BD}`,background:"#F8FAFC"}}><p style={{color:MUTED,fontSize:11,margin:0}}>Use o filtro de período para conferir pagamentos de uma data específica</p></div>}
              </div>
            </div>
          )}
          ...
          {tab==="perdas"&&(
            <div style={{display:"flex",flexDirection:"column",gap:24}}>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:20}}>
                {[{l:"Capital em Prejuízo",v:fmtR(perdas.capitalBaixado),c:RED},{l:"Recuperado",v:fmtR(perdas.recuperadoAposBaixa),c:PUR},{l:"Prejuízo Real",v:fmtR(perdas.prejuizoTotal),c:RED},{l:"Taxa de Recuperação",v:fmtP(perdas.txRecuperacao),c:GRN}].map(k=>(
                  <div key={k.l} style={{background:CARD,padding:20,borderRadius:12,border:`1px solid ${BD}`}}><div style={{fontSize:11,color:MUTED,fontWeight:600,textTransform:"uppercase",marginBottom:4}}>{k.l}</div><div style={{fontSize:20,fontWeight:800,color:k.c}}>{k.v}</div></div>
                ))}
              </div>
              <div style={{background:CARD,borderRadius:12,border:`1px solid ${BD}`,overflow:"hidden"}}>
                <div style={{padding:20,borderBottom:`1px solid ${BD}`,display:"flex",justifyContent:"space-between",alignItems:"center",background:BG+"50"}}>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",textAlign:"left"}}>
                  ...
                  <tbody>{pFiltradas.map(c=>(
                    <tr key={c.ID_CONTRATO} style={{borderBottom:`1px solid ${BD}`,fontSize:13}}>
                      ...
                      <td><Badge c={STATUS_COR[c.STATUS_CONTRATO]||MUTED}>{STATUS_LABEL[c.STATUS_CONTRATO]?.toUpperCase()||c.STATUS_CONTRATO}</Badge></td>
          ...
          {tab==="simulador"&&(
            <div style={{maxWidth:900,margin:"0 auto"}}>
              <div style={{background:CARD,borderRadius:16,border:`1px solid ${BD}`,padding:32,boxShadow:"0 10px 40px rgba(0,0,0,0.05)"}}>
                <div style={{textAlign:"center",marginBottom:32}}><h2 style={{fontSize:24,fontWeight:800,margin:"0 0 8px"}}>Simulador de Expansão</h2><p style={{color:MUTED,margin:0}}>Projete o crescimento da sua carteira e analise o risco</p></div>
