...
function App(){
  ...
  // Estados do calendário do Financeiro
  const [finDe,setFinDe]=useState(null);
  const [finAte,setFinAte]=useState(null);
  const [finCalOpen,setFinCalOpen]=useState(false);
  const [finCalMes,setFinCalMes]=useState(()=>{const d=new Date();d.setDate(1);return d;});
  const [finRangeStep,setFinRangeStep]=useState(0);
      ...
      <div style={{flex:1,marginLeft:SW,display:"flex",flexDirection:"column",minHeight:"100vh"}}>
        ...
        <main style={{flex:1,padding:"24px 28px",overflowY:"auto"}}>
          ...
          {tab==="financeiro"&&(()=>{
            ...
            const DIAS_SEM2=["D","S","T","Q","Q","S","S"];
            function dStr2(d){if(!d)return"";return d.toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric"});}
            function diasDoMes2(a,m){return new Date(a,m+1,0).getDate();}
            function primDiaSem2(a,m){return new Date(a,m,1).getDay();}

            const fAno=finCalMes.getFullYear(), fMes=finCalMes.getMonth();
            const totalDias2=diasDoMes2(fAno,fMes), primDia2=primDiaSem2(fAno,fMes);
            const celulas2=[];
            for(let i=0;i<primDia2;i++) celulas2.push(null);
            for(let i=1;i<=totalDias2;i++) celulas2.push(new Date(fAno,fMes,i));

            function clicarDia2(d){
              if(finRangeStep===0){setFinDe(d);setFinAte(null);setFinRangeStep(1);}
              else{
                if(d<finDe){setFinDe(d);setFinAte(finDe);}
                else setFinAte(d);
                setFinRangeStep(0);setFinCalOpen(false);
              }
            }
            function ehIni2(d){return finDe&&d.getTime()===finDe.getTime();}
            function ehFim2(d){return finAte&&d.getTime()===finAte.getTime();}
            function noRange2(d){return finDe&&finAte&&d>=finDe&&d<=finAte;}
            function ehHoje2(d){const hj=new Date();hj.setHours(0,0,0,0);return d.getTime()===hj.getTime();}

            // Filtra pagamentos pelo range selecionado
            const pagsFiltrados=([...pagamentos].sort((a,b)=>new Date(b.DATA_PAGAMENTO||0)-new Date(a.DATA_PAGAMENTO||0))).filter(p=>{
              if(!finDe) return true;
              const dt=p.DATA_PAGAMENTO;if(!dt)return false;
              const d=new Date(dt);d.setHours(0,0,0,0);
              const fim=finAte?new Date(finAte):new Date(finDe);fim.setHours(23,59,59,999);
              return d>=finDe&&d<=fim;
            });

            // Totais do período filtrado
            const totalPeriodo=pagsFiltrados.reduce((s,p)=>s+(p.VALOR_PAGO||0),0);
            const extraPeriodo=pagsFiltrados.reduce((s,p)=>s+(p.RECEITA_EXTRA_ATRASO||0),0);
            const lucroPeriodo=pagsFiltrados.reduce((s,p)=>s+(parseFloat(p.VALOR_JUROS)||0)+(p.RECEITA_EXTRA_ATRASO||0),0);
            const temFiltro=!!finDe;
            const labelFiltro=finDe&&finAte?`${dStr2(finDe)} até ${dStr2(finAte)}`:finDe?`A partir de ${dStr2(finDe)}`:"";
            ...
            {/* ── Conferência de Pagamentos por Período ── */}
            <div style={{background:CARD,borderRadius:10,border:`1px solid ${BD}`}}>
              <div style={{padding:"16px 20px",borderBottom:`1px solid ${BD}`,display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
                <h3 style={{fontSize:14,fontWeight:700,margin:0,flex:1}}>Conferência de Pagamentos</h3>

                {/* Seletor de período com calendário */}
                <div style={{position:"relative"}} onClick={e=>e.stopPropagation()}>
                  <button onClick={()=>{setFinCalOpen(o=>!o);setFinRangeStep(0);}}
                    style={{display:"flex",alignItems:"center",gap:6,padding:"7px 14px",borderRadius:8,border:`1.5px solid ${temFiltro?BLU:BD}`,background:temFiltro?"#EFF6FF":CARD,color:temFiltro?BLU:MUTED,fontSize:12,fontWeight:temFiltro?700:400,cursor:"pointer"}}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                    {temFiltro ? labelFiltro : "Filtrar por período"}
                  </button>

                  {finCalOpen&&<div style={{position:"absolute",top:"calc(100% + 8px)",right:0,zIndex:200,background:CARD,border:`1px solid ${BD}`,borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,0.14)",padding:16,minWidth:300}}>
                    <p style={{color:finRangeStep===0?BLU:GRN,fontSize:11,fontWeight:600,margin:"0 0 10px",textAlign:"center"}}>
                      {finRangeStep===0?"Clique na data inicial":"Clique na data final"}
                    </p>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                      <button onClick={()=>{const d=new Date(finCalMes);d.setMonth(d.getMonth()-1);setFinCalMes(d);}} style={{width:28,height:28,borderRadius:6,border:`1px solid ${BD}`,background:CARD,cursor:"pointer",color:MUTED,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
                      <span style={{fontWeight:700,fontSize:13,color:TEXT}}>{MESES_PT2[fMes]} {fAno}</span>
                      <button onClick={()=>{const d=new Date(finCalMes);d.setMonth(d.getMonth()+1);setFinCalMes(d);}} style={{width:28,height:28,borderRadius:6,border:`1px solid ${BD}`,background:CARD,cursor:"pointer",color:MUTED,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",marginBottom:4}}>
                      {DIAS_SEM2.map((d,i)=><div key={i} style={{textAlign:"center",fontSize:10,fontWeight:700,color:MUTED,padding:"2px 0"}}>{d}</div>)}
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
                      {celulas2.map((d,i)=>{
                        if(!d) return <div key={i}/>;
                        const ini=ehIni2(d),fim=ehFim2(d),dentro=noRange2(d),hj=ehHoje2(d);
                        const ativo=ini||fim;
                        return(
                          <button key={i} onClick={()=>clicarDia2(d)}
                    ...
                    {temFiltro&&<div style={{marginTop:12,paddingTop:10,borderTop:`1px solid ${BD}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:11,color:MUTED}}>{labelFiltro}</span>
                      <button onClick={()=>{setFinDe(null);setFinAte(null);setFinRangeStep(0);setFinCalOpen(false);}} style={{fontSize:11,color:RED,background:"none",border:"none",cursor:"pointer",fontWeight:600}}>Limpar</button>
                    </div>}
                  </div>}
                </div>

                {temFiltro&&<button onClick={()=>{setFinDe(null);setFinAte(null);setFinRangeStep(0);}} style={{padding:"6px 12px",borderRadius:8,border:`1px solid ${BD}`,background:CARD,color:MUTED,fontSize:12,cursor:"pointer"}}>✕ Limpar</button>}
              </div>

              {/* Totais do período */}
              {temFiltro&&<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:0,borderBottom:`1px solid ${BD}`}}>
                {[
                  {l:"Pagamentos no período",v:pagsFiltrados.length,c:TEXT,unit:""},
                  {l:"Total recebido",v:fmtR(totalPeriodo),c:BLU,unit:""},
                  {l:"Lucro (juros + extra)",v:fmtR(lucroPeriodo),c:GRN,unit:""},
                  {l:"Receita extra (atraso)",v:fmtR(extraPeriodo),c:ORG,unit:""},
                ].map((k,i)=>(
                  <div key={k.l} style={{padding:"12px 20px",borderRight:i<3?`1px solid ${BD}`:"none",background:"#F8FAFC"}}>
                    <p style={{color:MUTED,fontSize:10,textTransform:"uppercase",fontWeight:600,margin:"0 0 3px"}}>{k.l}</p>
              ...
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead><tr style={{background:BG}}>
                  {["Data","Cliente","Contrato","Tipo","Valor Original","Valor Pago","Extra Atraso"].map(h=>(
                    <th key={h} style={{textAlign:"left",padding:"10px 20px",color:MUTED,fontSize:11,fontWeight:700,textTransform:"uppercase"}}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {pagsFiltrados.length===0
                    ...
                    :pagsFiltrados.map((p,i)=>{
                      ...
                      return<tr key={i} style={{borderTop:`1px solid ${BD}`,background:i%2===0?CARD:"#FAFAFA"}}>
                        <td style={{padding:"11px 20px",color:MUTED,fontSize:12,whiteSpace:"nowrap"}}>{fmtDt(p.DATA_PAGAMENTO)}</td>
                        <td style={{padding:"11px 20px",fontSize:12,fontWeight:600}}>{p.NOME_CLIENTE}</td>
                        <td style={{padding:"11px 20px",fontSize:11,color:MUTED}}>{p.ID_CONTRATO||"—"}</td>
                        <td style={{padding:"11px 20px"}}><Badge c={tCor}>{tLabel}</Badge></td>
                        <td style={{padding:"11px 20px",color:MUTED,fontSize:12}}>{fmtR(p.VALOR_ORIGINAL_PARCELA)}</td>
                        <td style={{padding:"11px 20px",fontWeight:700,fontSize:13,color:BLU}}>{fmtR(p.VALOR_PAGO)}</td>
                        <td style={{padding:"11px 20px",color:p.RECEITA_EXTRA_ATRASO>0?ORG:MUTED,fontWeight:p.RECEITA_EXTRA_ATRASO>0?700:400,fontSize:12}}>
                          {p.RECEITA_EXTRA_ATRASO>0?`+${fmtR(p.RECEITA_EXTRA_ATRASO)}`:"—"}
                        </td>
                      </tr>;
                    })
                  }
              ...
              {!temFiltro&&<div style={{padding:"10px 20px",borderTop:`1px solid ${BD}`,background:"#F8FAFC"}}>
                <p style={{color:MUTED,fontSize:11,margin:0}}>Mostrando todos os {pagamentos.length} pagamentos — use o filtro de período para refinar</p>
              </div>}
            </div>
          </div>; })()}
