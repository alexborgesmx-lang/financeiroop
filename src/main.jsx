...
function NovoContrato({contratos,clientes,onSucesso,clienteInicial}){
  ...
  const [dtVenc,setDtVenc]=useState(()=>{const dia=clienteInicial?.DIA_VENCIMENTO_PREFERIDO||(clientes||[]).find(c=>String(c.ID_CLIENTE)===String(clienteInicial?.ID_CLIENTE))?.DIA_VENCIMENTO_PREFERIDO;return dia?calcProxVenc(dia):"";});const [loading,setLoading]=useState(false);const [msg,setMsg]=useState(null);const [contratoOk,setContratoOk]=useState(null);const [docLoading,setDocLoading]=useState(false);const [zapLoading,setZapLoading]=useState(false);const [zapUrl,setZapUrl]=useState("");const [zapErro,setZapErro]=useState("");const [zapWppUrl,setZapWppUrl]=useState("");const [zapSemLink,setZapSemLink]=useState(false);const [zapDocToken,setZapDocToken]=useState("");const [carneLoading,setCarneLoading]=useState(false);const [carneOk,setCarneOk]=useState(false);const [carneErro,setCarneErro]=useState("");const [criandoSteps,setCriandoSteps]=useState(null);const ref=useRef();
  useEffect(()=>{const h=e=>{if(ref.current&&!ref.current.contains(e.target))setShowDrop(false);};document.addEventListener("mousedown",h);return()=>document.removeEventListener("mousedown",h);},[]);
  ...
  const _gerarDocRetry=()=>{
    if(!contratoOk||docLoading)return;
    setDocLoading(true);setContratoOk(prev=>prev?{...prev,docErro:""}:prev);
    postAction({action:"gerarDoc",idContrato:contratoOk.idContrato,idCliente:contratoOk.clienteId,dados:contratoOk._dadosDoc}).then(d=>{
      if(d.ok)setContratoOk(prev=>prev?{...prev,docUrl:d.docUrl||"",docId:d.docId||""}:prev);
      else setContratoOk(prev=>prev?{...prev,docErro:d.erro||"Erro ao gerar documento"}:prev);
    }).catch(e=>setContratoOk(prev=>prev?{...prev,docErro:e.message}:prev)).finally(()=>setDocLoading(false));
  };
...
function ContratoModal({ contrato, parcelas, pagamentos, clientes, eventos, onRegistrarPagamento, onReagendar, onBaixar, onQuitacaoAntecipada, onComprovante, onAlterarVencimento, onFechar, onExcluir, onAcordoAssistido, onAbatimento, onVoltarCliente, onAjuizar, onRenegociar, onRecuperar, onAcordoJudicial, onQuitacaoJudicial, onArquivarProcesso }) {
  ...
  const lucroRecuperadoAssistido = parseFloat(contrato.LUCRO_RECUPERADO_ASSISTIDO||0);
            ...
            <div>
              {[
                ...
                ...(isAcordoAssistido?[
                  ...
                  ...(lucroRecuperadoAssistido>0?[{l:"Lucro recuperado (Acordo Assistido)",v:fmtR(lucroRecuperadoAssistido),c:GRN}]:[]),
