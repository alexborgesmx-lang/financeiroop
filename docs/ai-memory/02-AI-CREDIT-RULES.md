...
4. **Contabilização em cascata capital-primeiro (2026-08-29):** cada abatimento é alocado primeiro pro principal do contrato ainda em aberto (`_alocarAbatimentoAssistido`, `appscript.gs`) — só a parte que sobra depois do principal 100% recuperado é reconhecida como lucro. "Principal já recuperado" considera o histórico inteiro do contrato: soma do principal das parcelas pagas integralmente (excluindo `somente_juros`, que rola o principal em vez de devolvê-lo) mais os abatimentos anteriores. `VALOR_ABATIDO_ASSISTIDO` (CONTRATOS) guarda só a parte-principal acumulada; `LUCRO_RECUPERADO_ASSISTIDO` (CONTRATOS, novo) guarda a parte-lucro — **nunca somado a `LUCRO_TOTAL`** operacional, mesmo padrão já usado na Recuperação Judicial (`_alocarRecuperacaoJudicial`). Por pagamento, `CAPITAL_RECUPERADO_ASSISTIDO`/`LUCRO_RECUPERADO_ASSISTIDO` (PAGAMENTOS) guardam o detalhe daquele abatimento específico. O Dashboard e aba Financeiro continuam excluindo `abatimento_acordo_assistido` dos totais de receita — o reconhecimento de lucro aqui é só em `LUCRO_RECUPERADO_ASSISTIDO`, separado.
...
6. **Baixa usa VALOR_ABATIDO_ASSISTIDO**: ao calcular PREJUIZO_CAPITAL na baixa de um contrato `acordo_assistido`, usar `VALOR_ABATIDO_ASSISTIDO` como capital recuperado (não somar parcelas pagas, que serão zero). Com a cascata capital-primeiro, esse campo agora reflete só a parte-principal de fato recuperada — o cálculo de prejuízo fica mais preciso (antes, um abatimento que já devia contar como lucro reduzia indevidamente o prejuízo, como se fosse capital).
